...
import (
	"golang-api-template/helper/str"
	"net/http"
	"reflect"
...
// SendValidationCodeMessageStringError ...
// Send validation error response to consumers.
func (u *HTTPHelper) SendValidationStringError(c echo.Context, validationErrors validator.ValidationErrors) error {
	errorResponse := ""
	errorTranslation := validationErrors.Translate(u.Translator)
	for _, err := range validationErrors {
		errKey := str.Underscore(err.StructField())
	...
	return c.JSON(codeBadRequestError, map[string]interface{}{
		"code":         codeBadRequestError,
		"code_type":    "validationError",
		"code_message": errorResponse,
		"data":         u.EmptyJsonMap(),
	})
}
...
//Set paginantion response
func (u *HTTPHelper) GeneratePagingV2(c echo.Context, prev, next, limit, page, totalFiltered, totalRecord int) map[string]interface{} {
